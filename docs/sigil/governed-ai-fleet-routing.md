...
For later operator-run device preflight, run `apps/sigil/scripts/certify-governed-ai-fleet.py` with explicit node, role, authenticated identity reference, transport identity, provider/model, and timestamps. It performs no network access or configuration changes and emits only identity-reference digests and a sanitized report. Actual connectivity, read-only task, cancellation, failover, and evidence round-trip certification remains blocked until operators supply authenticated live Titan/Mac/Prime access; it must never be inferred from local mocks.
