...
def build_release_readiness(certification: SystemIntegrationCertification, *, repository_clean: bool, required_suites: Iterable[str], passed_suites: Iterable[str], failed_suites: Iterable[str] = (), skipped_suites: Iterable[str] = (), generated_at: int = 0, known_good_source_commit: str = "14272ebb1") -> ReleaseReadiness:
    required = tuple(sorted(set(required_suites))); passed = tuple(sorted(set(passed_suites))); failed = tuple(sorted(set(failed_suites))); skipped = tuple(sorted(set(skipped_suites)))
    ...
    disposition = ReleaseDisposition.FAILED if any(item.severity is FindingSeverity.CRITICAL for item in blocking) else (ReleaseDisposition.BLOCKED if blocking or failed or not repository_clean else (ReleaseDisposition.CONDITIONALLY_READY if advisory or risks or skipped else ReleaseDisposition.READY_FOR_OPERATOR_REVIEW))
