...
const dmg = path.join(release, 'Sigil-1.9.0-mac-arm64.dmg')
...
const report = {
  ...
  artifacts: {
    ...
    zip: fs.existsSync(path.join(release, 'Sigil-1.9.0-mac-arm64.zip')),
...
fs.writeFileSync(path.join(release, 'certification', 'v1.9.0-release.json'), JSON.stringify(report, null, 2) + '\n')
