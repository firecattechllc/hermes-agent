...
const dmg = path.join(release, 'Sigil-2.1.0-mac-arm64.dmg')
...
const report = {
  ...
  artifacts: {
    ...
    zip: fs.existsSync(path.join(release, 'Sigil-2.1.0-mac-arm64.zip')),
...
fs.writeFileSync(path.join(release, 'certification', 'v2.1.0-release.json'), JSON.stringify(report, null, 2) + '\n')
