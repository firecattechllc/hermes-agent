...
describe('Sigil Electron startup', () => {
  ...
  it('keeps provider access in a bounded backend-only IPC channel', () => {
    ...
    expect(mainSource).toContain("SIGIL_AI_STATUS_CHANNEL = 'sigil:get-ai-status'")
    expect(mainSource).toContain("runBridgeRequest({ command: 'ai_status' })")
    expect(preloadSource).toContain('getAIStatus')
    expect(preloadSource).not.toContain('invokeAI')
    expect(preloadSource).not.toContain('prompt')
